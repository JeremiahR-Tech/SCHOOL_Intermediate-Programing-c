...
int matches_key(const void *elem, char *key) {
    ...
    // String only for debug statement
    char debug[HM_DEBUG_SIZE] = { 0 };
    sprintf(debug,"hash_map_utils.c::matches_key: hash_elem->key = %s, key = %s\n", hash_elem->key, key);
    printd(debug);
...
//    map->r_value = (int) ceil(1.0 / (DEFAULT_RESIZE_FACTOR - 1));
   map->r_value = RVALUE(DEFAULT_RESIZE_FACTOR);
}
...
hash_element_t *search(hash_map_t *map, char *key) {
	...
	char debug[HM_DEBUG_SIZE] = { 0 };
        sprintf(debug,"hash_map_utils.c::search: elem = %p\n", elem);
	printd(debug);
...
void rehash_inc(hash_map_t *map) {
	...
	char debug[HM_DEBUG_SIZE] = { 0 };
        sprintf(debug,"Rehashing %s into %d\n", elem->key, index);
	printd(debug);
...
void insert(hash_map_t *map, hash_element_t *elem) {
    ...
    char debug[HM_DEBUG_SIZE] = { 0 };
    sprintf(debug,"factor = %.2lf", factor);
    printd(debug);

    if (factor >= LOAD_FACTOR) {
        sprintf(debug,"Beginning rehash...");
	printd(debug);
        // Begin rehash
        resize_map(map, DEFAULT_RESIZE_FACTOR);
    }

    sprintf(debug,"Inserting %s at %d", elem->key, index);
    printd(debug);
