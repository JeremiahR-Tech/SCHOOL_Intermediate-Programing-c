...
void *pop_item(dynamic_array_t *array, int index) {
    ...
    // Buffer used only for debug statements
    char debug[DA_DEBUG_SIZE] = { 0 };
    sprintf(debug,"dynamic_arrays.c::pop_item: index = %d, array->size = %d\n", index, array->size);
    printd(debug);
...
void *find_item(dynamic_array_t *array, char *key, int (*check_fn)(const void *, char *)) {
    for (int i = 0; i < array->size; i++) {
	    ...
	    char debug[DA_DEBUG_SIZE] = { 0 };
            sprintf(debug,"dynamic_arrays.c::find_item: Found item %s\n", key);
	    printd(debug);
            return array->data[i];
        }
    }
